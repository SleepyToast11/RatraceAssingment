...
final class RatFrame extends JFrame implements ActionListener, ChangeListener
{
  ...
  // INSTANCE VARIABLES
  private int FPS = 5;
  private final JLabel ratLabel = new JLabel();
  private final JComboBox cbox;
  private final JSlider slider = new JSlider(JSlider.HORIZONTAL, 0, MAX_FPS, FPS);
  private final JFileChooser mazeChooser = new JFileChooser(System.getProperty("user.dir"));
  private final Vector ratTypes = new Vector();
  private Class ratClass = null;
  private final javax.swing.Timer timer = new javax.swing.Timer(1000 / FPS, this);
  private final RatPanel ratpanel = new RatPanel();
  ...
  private final void loadRat() throws Exception
  {
    ...
    {
      Constructor ctor = ratClass.getConstructor();
  ...
  private final class RatPanel extends JPanel
  {
    ...
    public void paintComponent(Graphics g)
    {
      ...
      int fontSize = Math.min(colWidth, rowHeight);
      ...
      for (int row = 0;  row < numRows;  row++)
        for (int col = 0;  col < numCols;  col++)
        {
          ...
          else if (square == Maze.WALL)
          {
            ...
            g2.fill3DRect(x, y, colWidth, rowHeight, true);
          }
        }
      ...
      int x = col * colWidth;
      int y = row * rowHeight;
      g2.fillOval(x, y, colWidth, rowHeight);
